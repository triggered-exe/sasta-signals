export const productQueries = {
  SnacksAndBeverages: {
    Snacks: ["Chips", "Namkeens", "Biscuits", "Chocolates", "Nuts"],
    Beverages: ["Tea", "Coffee", "Juices", "Soft Drinks", "Energy Drinks"],
    Sweets: ["ice creams", "candies", "indian sweets"],
    Concentrates: ["syrups"],
  },
  MedicinesAndHealthcare: {
    HealthSupplements: [
      "multi vitamins",
      "Protein powder",
      "Creatine",
      "Pre-workout",
      "BCAA",
    ],
    FitnessFoods: [
      "Energy Bars",
      "Protein Bars",
      "Sugar-Free",
    ],
  },
  HealthySpecialDietFoods: {
    PlantBasedProtein: ["Tofu", "soya", "plant protein"],
    DryFruitsNuts: [
      "Dry Fruits",
    ],
  },
  Groceries: {
    Staples: ["Rice", "Wheat Flour", "Pulses", "Sugar", "Salt", "Cooking Oil"],
    SpicesAndCondiments: ["Spices", "Masala"],
    CannedAndPackagedFoods: [
      "Pasta",
      "Noodles",
      "Sauces",
      "Pickles",
      "Jams",
    ],
  },
  FreshFruitsAndVegetables: {
    Fruits: ["fruits"],
    Vegetables: ["vegetables"],
  },
  DairyProducts: {
    Milk: ["milk", "milk powder"],
    Cheese: ["cheese", "paneer"],
    YogurtAndCurd: ["curd", "yogurt"],
    ButterAndGhee: ["butter", "ghee"],
  },
  BakeryAndBreakfastItems: {
    Bread: ["Bread"],
    BreakfastCereals: ["Cornflakes", "Oats", "Muesli", "Granola", "cereals"],
  },
  SnacksAndBeverages: {
    Snacks: ["Chips", "Namkeens", "Biscuits", "Chocolates", "Nuts"],
    Beverages: ["Tea", "Coffee", "Juices", "Soft Drinks", "Energy Drinks"],
    Sweets: ["ice creams", "candies", "indian sweets"],
    Concentrates: ["syrups"],
  },
  PersonalCare: {
    HairCare: ["Shampoo", "Conditioner", "Hair Oil"],
    SkinCare: ["Face Wash", "Moisturizer", "Sunscreen", "Face Creams"],
  },
  FrozenFoods: {
    ReadyToEatMeals: ["Frozen foods"],
  },
  Electronics: {
    ComputerAccessories: ["computer accessories"],
    MobileAccessories: ["chargers", "earphones", "power banks", "headphones", "speakers"],
    SmallAppliances: ["home appliances", "kitchen appliances", "personal care appliances", "office appliances"],
    Batteries: ["batteries"],
  },
  StationeryAndOfficeSupplies: {
    Stationery: ["stationery"],
  },
  Fashion: {
    Apparel: ["T-Shirts", "Jeans", "Dresses", "Shirts"],
  },
  BeautyAndWellness: {
    Fragrances: ["perfumes", "deodorants"],
  },
  // KitchenEssentials: {
  //   Cookware: ["Cookware"],
  //   Utensils: ["Utensils"],
  //   Storage: ["Containers"],
  // },
  // CleaningSupplies: {
  //   HomeCleaning: ["Cleaner"],
  // },
  // SeasonalItems: {
  //   FestiveDecor: ["rangoli colors", "diya lamps", "festive lights"],
  //   Gifts: ["gifts"],
  // },
  // PartyAndEntertainment: {
  //   PartySupplies: [
  //     "Party",
  //   ],
  //   QuickSnacks: ["Party Mix"],
  // },
  // //   BabyCare: {
  //     Diapers: ["Disposable Diapers", "Cloth Diapers"],
  //     BabyFood: ["Infant Formula", "Baby Cereal", "Purees"],
  //     BabyAccessories: ["Baby Wipes", "Baby Bottles", "Pacifiers"],
  //   },
  //   PetSupplies: {
  //     PetFood: ["Dog Food", "Cat Food", "Bird Food", "Fish Food"],
  //     PetAccessories: ["Toys", "Collars", "Leashes", "Litter Boxes"],
  //   },
//   HomeDecorAndGardening: {
//     Decor: ["candles", "photo frames", "vases"],
//     Gardening: ["seeds", "pots", "fertilizers", "gardening tools"],
//   },
};
