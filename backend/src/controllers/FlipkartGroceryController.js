...
export const searchProductsUsingCrawler = async (req, res, next) => {
  ...
  try {
    ...
    // Since input is already focused, just type pincode
    ...
    // Verify location is set by checking for if input still exists
    const locationInput = await page.$('input[placeholder*="Enter pincode"]');

    if (locationInput) {
      throw new AppError("Location not servicable or not set", 400);
    }
    ...
    while (hasNextPage) {
        ...
        return Array.from(productElements)
          .map((element) => {
            ...
            return {
              ...
              url: nameElement
                ? "https://www.flipkart.com" + nameElement.getAttribute("href")
                : "",
          ...
          .filter((product) => product.productName && product.url);
      });

      // Add products from current page to all products
      allProducts = [...allProducts, ...pageProducts];

      console.log(`FK: Found ${pageProducts.length} products on current page`);

      // Check for next page
      const nextPageUrl = await page.evaluate(() => {
        // Find all buttons with the class _9QVEpD
        const paginationButtons = document.querySelectorAll("a._9QVEpD");

        // Convert to array and find the one containing "Next"
      ...
      if (nextPageUrl) {
        ...
        console.log("FK: Moving to next page:", currentUrl);
      } else {
        hasNextPage = false;
    ...
    next(
      error instanceof AppError
        ? error
        : AppError.internalError("Failed to fetch Flipkart products")
    );
  } finally {
