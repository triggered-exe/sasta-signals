...
export const searchProductsUsingCrawler = async (req, res, next) => {
    ...
    try {
        ...
        next(
            error instanceof AppError ? error : new AppError(error.message || "Failed to fetch Flipkart products", 500)
        );
    } finally {
        if (page) {
            await page.close();
                    ...
                    try {
                        ...
                        try {
                            ...
                            while (hasNextPage) {
                                ...
                                // Extract products using the new function
                                const { products: pageProducts, nextPageUrl } = await extractProductsFromPage(
                                    page,
                                    currentUrl,
                                    query
                                );
