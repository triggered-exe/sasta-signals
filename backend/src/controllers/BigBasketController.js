  ...
  try {
    ...
    while (hasMorePages) {
      const searchResponse = await axios.get(
        `https://www.bigbasket.com/listing-svc/v2/products?type=ps&slug=${encodeURIComponent(
          query
        )}&page=${currentPage}`,
        {
          headers: {
    ...
    next(
      error instanceof AppError ? error : AppError.internalError(`Failed to fetch BigBasket products, error: ${error}`)
    );
  }
};
    ...
    const transformedProducts = products
      ...
      .map((product) => {
        ...
        return {
          ...
          mrp: mrp,
          price: currentPrice,
          discount: Math.floor(((mrp - currentPrice) / (mrp || 1)) * 100),
          weight: product.w,
          brand: product.brand?.name,
          url: `https://www.bigbasket.com${product.absolute_url}`,
          eta: product.availability?.medium_eta,
  ...
  try {
    ...
    if (isNewLocation) {
      ...
      const clickResult = await page.evaluate(() => {
        const spans = Array.from(document.querySelectorAll("span")).filter(
          (span) => span.textContent.trim() === "Select Location"
        );
        if (spans.length > 0) {
          spans[0].click();
    ...
    while (hasMore) {
      hasMore = await page.evaluate(async () => {
        ...
        if (newCount >= 100) {
          parentContainer.children[parentContainer.children.length - 1].scrollIntoView({
            behavior: "smooth",
            block: "center",
          });
      ...
      return Array.from(parentContainer.children).map((item) => {
        ...
        return {
          ...
          url: item.querySelector("a")
            ? "https://www.bigbasket.com" + item.querySelector("a").getAttribute("href")
            : "",
